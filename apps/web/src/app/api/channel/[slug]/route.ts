...
export const GET = withLoggerAndErrorHandler(async (req: NextRequest, { params }: { params: { slug: string } }) => {
  ...
  try {
    ...
    if (cached) {
      return successResponse("Channel fetched (cache)", 200, JSON.parse(cached));
    }
    ...
    const isBanned = ban ? (!ban.expiresAt || ban.expiresAt > new Date()) : false;
